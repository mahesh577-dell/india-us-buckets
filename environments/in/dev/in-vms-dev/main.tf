# =================================================================  
# ENVIRONMENT: in/dev/in-vms-dev   (service project: india-vms-dev)
#
# Deploys into the Shared VPC owned by in/dev/in-host-dev-network.
...
# created by in/dev/in-host-dev-network. If you're not sure what a subnet
...
module "db" {
  ...
  region                = var.region
  instance_name         = "india-vms-dev-db"
...
# from the secondary ranges that in/dev/in-host-dev-network already reserved
...
module "gke" {
  ...
  project_id   = var.project_id
  region       = var.region
  cluster_name = "india-vms-dev-gke"
...
# forwarding pipeline / Atlas integration). Plain regional bucket -
# not related to the Terraform state bucket the bootstrap step creates.
module "email_ingestion_bucket" {
